...
sub find_binaries_tar()
{
	...
	@files;
	chomp(@PatchFiles);
	$dir="/data/ccmbm/provident/Provident_Delivery-patch_linAS4_7.4.0";
	my $ftpdir="/u/prathish/Patch_$patch_number";
	my %hash;
	foreach (@PatchFiles)
	{
	        ...
	        push(@dirs,dirname($_));
        	$sourcedir=$destdir=$_;
	        $sourcedir=~ s/\$PROVHOME/$dir\/Provident_Delivery/g;
        	$destdir=~s/\$PROVHOME/$ftpdir/g;
...
sub ffind_binaries_tar()
{
	...
	{	
        	...
        	print "$sourcedir and $destdir ";
	        $hash{$sourcedir}=$destdir;
        	#push(@sourcedirs,$PatchFile);
	}
	#Search the binaries in the delivery project
	print "\@dirs is: @dirs and \@files is: @files \n";
	foreach $dir(@dirs)
	{
        	$dir=~s/\$PROVHOME/$ftpdir/g;
	        #push(@newdir,$dir);
...
sub reconfigure_dev_proj_and_compile()
{
	...
	$PROV_LINAS4_DIR="Provident_Dev-patch_linAS4_".$PatchReleaseVersion;
	$projectName=$PROV_LINAS4_DIR;
	...
	# Reconfigure the project
	# Provident_Dev-patch_linAS4_7.4.0 9996
...
sub reconfigure_del_project()
{
	...
	$PatchReleaseVersion="7.4.0";
	$PROV_LINAS4_DIR="Provident_Delivery-patch_linAS4_".$PatchReleaseVersion;
	$projectName=$PROV_LINAS4_DIR;
	$ccmworkarea=`$CCM wa -show -recurse $projectName`;
	($temp,$workarea)=split(/'/,$ccmworkarea);
